...
export function useLinksDatabase() {
  ...
  async function searchByName(name: string, categoryId?: number) {
    try {
      ...
      if (categoryId) {
        query = 'SELECT * FROM links WHERE name LIKE ? AND category_id = ' + categoryId
      }

      const response = await database.getAllAsync<LinkDatabase>(query, `%${name}%`)
  ...
  async function remove(id: number) {
    const statement = await database.prepareAsync('DELETE FROM links WHERE id = $id')
