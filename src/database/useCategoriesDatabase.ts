...
export function useCategoriesDatabase() {
  ...
  async function searchByName(name: string) {
    try {
      const query = 'SELECT id, name FROM categories WHERE name LIKE ? ORDER BY name'
  ...
  async function remove(id: number) {
    const statement = await database.prepareAsync('DELETE FROM categories WHERE id = $id')
