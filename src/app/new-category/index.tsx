...
export default function NewCategory() {
  ...
  const [categorySelected, setCategorySelected] = useState<CategoryDatabase | null>(null)
  ...
  async function handleSave() {
    try {
      ...
      if (error instanceof ZodError) {
        return Alert.alert(t('alert_save_error_validation_title'), error.errors[0].message)
      }
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.form}>
        <Input placeholder={t('input_name_placeholder')} onChangeText={setName} defaultValue={categorySelected?.name} />
        <Button title={t('button_salve_title')} onPress={handleSave} isLoading={isLoading} />
      </View>
