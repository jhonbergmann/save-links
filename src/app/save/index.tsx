...
import {Alert, FlatList, Keyboard, Modal, Text, TouchableOpacity, View} from 'react-native'
...
export default function Save() {
  ...
  const formScheme = z.object({
    ...
    name: z.string({message: t('validation_required_name')}).min(1, {message: t('validation_required_name')}),
    url: z.string({message: t('validation_required_url')}).url({message: t('validation_invalid_url')}),
  })
  ...
  async function handleSave() {
    try {
      ...
      Alert.alert(t('alert_save_success_title'), t('alert_save_success_message'), [
        {
          text: 'Ok',
          onPress: () => router.back(),
        },
  ...
  async function getCategories() {
    try {
      const response = await categoriesDatabase.searchByName(searchCategoryByName)
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.form}>
        ...
        <Input placeholder={t('input_name_placeholder')} value={name} onChangeText={setName} />
        <Input placeholder={t('input_link_placeholder')} value={url} onChangeText={setUrl} />
        <Button title={t('button_salve_title')} onPress={handleSave} isLoading={isSaving} />
      </View>
      ...
      <Modal visible={modalIsOpen}>
        <View style={styles.modalContent}>
          ...
          <View style={styles.search}>
            <Input placeholder={t('input_search_placeholder')} onChangeText={setSearchCategoryByName} />
          </View>
          ...
          <Button title={t('button_category_add_title')} onPress={handleNewCategory} />
        </View>
      </Modal>
    </View>
  )
