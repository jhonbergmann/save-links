...
import {Alert, View, Text, Modal, Image, Linking, FlatList, Pressable, TouchableOpacity} from 'react-native'
...
import {LinkDatabase, LinkShowDatabase, useLinksDatabase} from '@/database/useLinksDatabase'
...
export default function Index() {
  ...
  async function getLinks() {
    try {
      const response = await linksDatabase.searchByName(searchLinkByName, category?.id)
      setLinks(response)
    } catch (error) {
      console.log(error)
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.search}>
        <Input placeholder={t('input_search_placeholder')} onChangeText={setSearchByName} />
      </View>
      ...
      <FlatList
        ...
        renderItem={({item}) => (
          <Category name={item.name} isSelected={item.id === category?.id} onPress={() => handleCategorySelect(item)} />
        )}
        horizontal
      ...
      <FlatList
        ...
        ListEmptyComponent={() => <Text style={styles.emptyList}>{t('flatlist_links_empty')}</Text>}
      />
        ...
        <Pressable style={styles.modal} onPress={() => setShowModal(false)}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.footer}>
              <Option name={t('option_edit_name')} icon="edit" variant="secondary" onPress={handleEdit} />

              <Option name={t('option_open_name')} icon="language" onPress={handleOpen} />
            </View>
          </View>
        </Pressable>
      </Modal>
